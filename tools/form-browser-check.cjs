...
(async()=>{
 ...
 const openFromMore=async(action)=>{await page.locator('.dock [data-action=more]').click();await page.locator(`#dialog [data-action=${action}]`).click();};
 ...
 await page.locator('.dock [data-action=coach]').click();assert.ok(await page.locator('#coach-page-form').isVisible());await page.locator('[data-action=coach-open]').click();assert.ok(await page.locator('#coach-dialog').isVisible());await page.locator('[data-action=coach-expand]').click();await page.locator('[data-action=coach-close]').click();await page.locator('.dock [data-action=today]').click();
 ...
 await page.locator('[data-action=close]').click();await openFromMore('routines');await page.locator('[data-action=new-routine]').click();await page.locator('#routine-name').fill('Test upper');await page.locator('[data-action=routine-add]').click();await page.locator('#exercise-search').fill('bench press');await page.locator('[data-pick=bench-press]').click();await page.locator('[data-action=routine-save]').click();assert.ok((await read()).routines.some(r=>r.name==='Test upper'));
 ...
 if(process.env.FORM_PRIVATE_PROFILE){
   await openFromMore('settings');await page.locator('#personal-file').setInputFiles(process.env.FORM_PRIVATE_PROFILE);await page.locator('[data-action=personal-confirm-import]').click();await page.waitForTimeout(200);
   ...
   await page.evaluate(async()=>{await(await import('/js/store.js')).discard();});await openFromMore('routines');await page.locator('[data-action=start][data-id=personal-monday-legs]').click();
